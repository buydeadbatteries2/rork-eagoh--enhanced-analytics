...
export default function LabsScreen(): JSX.Element {
  ...
  const renderStep = (): JSX.Element => {
    if (activeStep === "Identity") return <View style={styles.stepPanel}><SectionTitle eyebrow="01 / IDENTITY" title="Name the intelligence entity." /><TextInput value={forgeState.name} onChangeText={(value) => setSingle("name", value)} placeholder="Enter EAGOH name" placeholderTextColor={palette.muted} style={styles.input} /><SectionTitle eyebrow="SPORT MATRIX" title="Select a primary sport." /><FlatList data={sports} keyExtractor={(item) => item.id} renderItem={({ item }) => <OptionChip option={item} selected={forgeState.sport === item.id} onPress={(id) => setSingle("sport", id)} />} scrollEnabled={false} numColumns={2} columnWrapperStyle={styles.gridRow} contentContainerStyle={styles.gridList} /><SectionTitle eyebrow="FORM FACTOR" title="Choose gender expression." /><FlatList data={genders} keyExtractor={(item) => item.id} renderItem={({ item }) => <OptionChip option={item} selected={forgeState.gender === item.id} onPress={(id) => setSingle("gender", id)} />} scrollEnabled={false} numColumns={2} columnWrapperStyle={styles.gridRow} contentContainerStyle={styles.gridList} /></View>;
  ...
  const labSections = useMemo<Array<{ id: string }>>(() => [{ id: "content" }], []);
