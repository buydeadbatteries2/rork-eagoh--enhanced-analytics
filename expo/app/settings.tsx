...
import {
  upsertKnowledgeCredentials,
  getKnowledgeCredentials,
  deleteKnowledgeCredentials,
...
// ── Knowledge Credentials Form ──────────────────────────────────────────────

const CREDIBILITY_TAG_OPTIONS = [
  "Former athlete", "Music producer", "Film critic", "Fashion stylist",
  "Teacher", "Analyst", "Coach", "Content creator",
  "Industry professional", "Hobby expert",
];

const KnowledgeCredentialsForm = memo(function KnowledgeCredentialsForm({
  pal,
}: {
  pal: P;
}): JSX.Element {
  ...
  const [relevantBackground, setRelevantBackground] = useState("");
  ...
  useEffect(() => {
    if (!user?.id) return;
    setLoading(true);
    getKnowledgeCredentials(user.id)
      .then((row) => {
        if (row) {
          setCredentials(row);
          setPublicTitle(row.public_title ?? "");
          setDomainExpertise(row.domain_expertise ?? "");
          setExperienceSummary(row.experience_summary ?? "");
          setAccolades(row.accolades ?? "");
          setRelevantBackground(row.relevant_background ?? "");
          setYearsExperience(row.years_experience ? String(row.years_experience) : "");
          setSelectedTags(row.credibility_tags ?? []);
        }
        setLoading(false);
      })
  ...
  const handleSave = useCallback(async () => {
    ...
    setSaving(true);
    try {
      const input: KnowledgeCredentialsInput = {
        ...
        accolades: accolades.trim() || null,
        relevant_background: relevantBackground.trim() || null,
        years_experience: yearsExperience ? parseInt(yearsExperience, 10) || null : null,
        credibility_tags: selectedTags,
        is_public: true,
      };
      const row = await upsertKnowledgeCredentials(user.id, input);
      setCredentials(row);
      h.success();
      Alert.alert("Saved", "Your knowledge credentials have been updated.");
    } catch (err: unknown) {
      Alert.alert("Error", (err as Error).message ?? "Failed to save credentials.");
    } finally {
      setSaving(false);
    }
  }, [user?.id, publicTitle, domainExpertise, experienceSummary, accolades, relevantBackground, yearsExperience, selectedTags, h]);

  const handleDelete = useCallback(() => {
    if (!user?.id) return;
    Alert.alert(
      "Remove Credentials",
      "This will remove all your knowledge credentials. Your EAGOHs and listings are not affected.",
      [
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            try {
              await deleteKnowledgeCredentials(user.id);
              setCredentials(null);
              setPublicTitle("");
              setDomainExpertise("");
  ...
  const inlineStyles = useMemo(
    () => ({
      ...
      fieldLabel: { color: pal.muted, fontSize: 11, fontWeight: "900" as const, letterSpacing: 0.5, marginBottom: 4 },
      textInput: {
        color: pal.text,
        fontSize: 13,
        fontWeight: "700" as const,
      ...
      tagChipText: { color: pal.muted, fontSize: 11, fontWeight: "800" as const },
      tagChipTextActive: { color: pal.cyan },
      saveBtn: {
        minHeight: 44,
        borderRadius: 5,
        backgroundColor: pal.cyan,
      ...
      saveBtnText: { color: pal.void, fontSize: 14, fontWeight: "900" as const },
      deleteBtn: {
        minHeight: 40,
        borderRadius: 5,
        alignItems: "center" as const,
      ...
      deleteBtnText: { color: pal.ember, fontSize: 12, fontWeight: "900" as const },
    }),
    [pal],
  );
  ...
  return (
    <View style={inlineStyles.container}>
      {/* Helper text for privacy */}
      <View style={inlineStyles.helperText}>
        <Text style={{ color: pal.ember, fontSize: 11, fontWeight: "900" }}>
          Only add information you are comfortable showing publicly. Do not include private personal details.
        </Text>
      </View>

      {/* Public Display Title */}
      <View>
        <Text style={inlineStyles.fieldLabel}>Public Display Title</Text>
        <TextInput
          style={inlineStyles.textInput}
          value={publicTitle}
      ...
      {/* Domain Expertise */}
      <View>
        <Text style={inlineStyles.fieldLabel}>Domain Expertise</Text>
        <TextInput
          style={inlineStyles.textInput}
      ...
      {/* Experience Summary */}
      <View>
        <Text style={inlineStyles.fieldLabel}>Experience Summary</Text>
        <TextInput
      ...
      <View>
        <Text style={inlineStyles.fieldLabel}>Accolades / Achievements</Text>
        <TextInput
          style={inlineStyles.textArea}
          value={accolades}
      ...
      {/* Relevant Background */}
      <View>
        <Text style={inlineStyles.fieldLabel}>Relevant Background</Text>
        <TextInput
          style={inlineStyles.textArea}
          value={relevantBackground}
          onChangeText={setRelevantBackground}
      ...
      {/* Years of Experience */}
      <View>
        <Text style={inlineStyles.fieldLabel}>Years of Experience</Text>
        <TextInput
          style={inlineStyles.textInput}
      ...
      {/* Public Credibility Tags */}
      <View>
        <Text style={inlineStyles.fieldLabel}>Public Credibility Tags</Text>
        <View style={inlineStyles.tagsWrap}>
          {CREDIBILITY_TAG_OPTIONS.map((tag) => {
            const active = selectedTags.includes(tag);
            return (
              <Pressable
                key={tag}
                onPress={() => toggleTag(tag)}
                style={[inlineStyles.tagChip, active && inlineStyles.tagChipActive]}
              >
                <Tag color={active ? pal.cyan : pal.muted} size={12} />
                <Text style={[inlineStyles.tagChipText, active && inlineStyles.tagChipTextActive]}>
                  {tag}
                </Text>
              </Pressable>
            );
      ...
      >
        {saving ? (
          ...
          <>
            <PlusCircle color={pal.void} size={16} />
            <Text style={inlineStyles.saveBtnText}>
      ...
      {credentials && (
        <Pressable
          ...
          style={({ pressed }) => [
            inlineStyles.deleteBtn,
            pressed && { opacity: 0.7 },
          ]}
        >
          <Trash2 color={pal.ember} size={14} />
          <Text style={inlineStyles.deleteBtnText}>Remove Credentials</Text>
        </Pressable>
      )}
    </View>
  );
  ...
  const sections: SettingsSection[] = useMemo<SettingsSection[]>(() => {
    const secs: SettingsSection[] = [
      ...
      },
      {
        ...
        titleIcon: <BookOpen color={pal.cyan} size={15} />,
        rows: [
          {
            kind: "custom" as const,
            render: () => <KnowledgeCredentialsForm pal={pal} />,
          },
        ],
      },
      {
